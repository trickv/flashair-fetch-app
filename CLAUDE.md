...
- **iOS** — first built/run on 2026-05-27 (Xcode 26.5, iOS 16 deployment target); validated against a real Toshiba FlashAir card on 2026-05-29; **full-card real-hardware stress test on 2026-05-31: 170 files / 768.8 MB / 0 failures / ~9m39s (~3.4s/file ≈ 10.6 Mbps effective)**, followed by an immediate re-sync that correctly reported "170 already synced, 0 new" — dedupe-at-scale verified. Sentry telemetry wired with privacy-conscious defaults (opt-in default-on; `Utils/Telemetry.swift` wrapper). Build commit + date shown in Settings → About (postBuildScripts injects into bundle Info.plist). The full M2 sync path works on both mock *and* real hardware: `NEHotspotConfiguration` join (Hotspot entitlement provisions under a paid Apple Developer team), `/DCIM` walk (Toshiba uses `100__TSB` for the camera subdir, not `100CANON`; sibling `EOSMISC` + `MISC` dirs from the Canon firmware are tolerated), cleartext HTTP downloads, save to Photos with original filename preserved via `PHAssetResourceCreationOptions.originalFilename` (Immich/iCloud/Google Photos friendly, EXIF intact), `path#size` dedupe with the index persisted to `Documents/FlashAirSync/sync_index_<ssid>.json`, post-sync Wi-Fi teardown. Tailscale split-route did **not** divert private-IP traffic into the tunnel. **Debug workflow:** `xcrun devicectl device process launch --console` over USB tether streams `print()`/`Logger` output live and survives the Wi-Fi handoff to the FlashAir; the same tunnel over Wi-Fi pairing does not (it's severed the moment NEHotspotConfiguration switches networks). See `docs/REAL-WORLD-FINDINGS.md` for the full catalog of operational discoveries. Next iOS work is the deferred-cleanups list below (and `TODO.md`).
...
See `README.md` and `DEVELOPMENT.md` for the running roadmap; `TODO.md` for the active backlog with effort estimates; `docs/PARITY.md` for the iOS↔Android feature mirror checklist; `docs/REAL-WORLD-FINDINGS.md` for what real hardware taught us; `docs/BETA-TESTING.md` for distribution setup. `SESSION-2025-01-18.md` is a historical session log (predates the M2 merge).
...
- `/DCIM` usually contains only camera-vendor subdirectories — and the convention varies by manufacturer. Canon DSLRs create `100CANON`; the **Toshiba FlashAir we tested with creates `100__TSB`** as its own DCIM-compliant subdir, plus sibling `EOSMISC` + `MISC` dirs from the Canon firmware. Photos are one level deeper. A listing showing "3 directories, 0 files" at `/DCIM` is correct, not a bug. The walker must be name-agnostic — don't hardcode `100CANON`.
- Real-world throughput on a Toshiba FlashAir W-04: **~10.6 Mbps effective, ~3.4s/file steady-state**. The first ~30s of any sync is dominated by Wi-Fi join + recursive `/DCIM` scan; per-file downloading hits steady state after that. URLSession's observed 30.5s timeout matches the card — use 30s as a floor for HTTP timeouts on both platforms.
- **Camera body sleep is a hard constraint.** Cameras (Canon T5i default: 2 min) idle-sleep and power down their Wi-Fi. The card has no API to wake the camera body. User workaround: bump camera power-save to 15 min / never, or periodic shutter half-press.
